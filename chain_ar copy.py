...
def run_etl(condate):
    try:
        ...
        INSERT INTO tb_chain_ar (
            chain_no, JUNMISU, FOODLINEAMT, ROYALTYAMT, POSAMT, ADVAMT, ETCAMT, 
        ...
        for row in sales_data:
            # chain_no, foodlineamt, royaltyamt, posamt, advamt, etcamt, cashamt, cardamt, ticketamt = row
            chain_no = row[0]
            foodlineamt = row[1] or 0
            royaltyamt = row[2] or 0
            posamt = row[3] or 0
            advamt = row[4] or 0
            etcamt = row[5] or 0
            cashamt = row[6] or 0
            cardamt = row[7] or 0
            ticketamt = row[8] or 0
            junmisuamt = junmisu_data.get(chain_no, 0)  # 전월 미수금, 없으면 0
            misu = junmisuamt + (foodlineamt + royaltyamt + posamt + advamt + etcamt) - (cashamt + cardamt + ticketamt)  # 미수금 계산
            # misu = 0
            mysql_cursor.execute(mysql_insert_query, (
                chain_no, junmisuamt, foodlineamt, royaltyamt, posamt, advamt, etcamt, 
                cashamt, cardamt, ticketamt, misu
            ))
            inserted_rows += 1
