...
def convert_to_utf8(value):
    if isinstance(value, str):
        try:
            ...
            return temp_byte.decode('euc-kr')  # Convert to UTF-8 for proper Korean display
        except Exception as e:
...
def save_to_mysql(data):
    ...
    # cursor.execute(f"truncate tb_stock_current")

    insert_query = """
    INSERT INTO tb_stock_current 
    (item_no, full_name, unit, standard, pres_qty, pres_amt, ins_qty, ins_amt, outs_qty, outs_amt, pqty, pcosts, stock_date, created_at) 
    ...
    for row in data:
        try:
            ...
            # Log the full_name before conversion
            # logging.info(f"Informix full_name: {row[1]}")

            # Convert relevant fields to UTF-8
            row_utf8 = tuple(convert_to_utf8(item) for item in row)

            # Log the full_name after conversion
            # logging.info(f"UTF-8 converted full_name: {row_utf8[1]}")
            ...
            # Log the full_name before MySQL insert
            # logging.info(f"Prepared full_name for MySQL insert: {row_utf8[1]}")
